...
class Net(nn.Module):
    ...
    def entropy(self, x):
        ...
        out = _x * logx
        out = torch.sum(out, 1)
        out = out[:, None] #Add dimension for shap package
        return -out
...
if __name__ == '__main__':
    ...
    mode = 'test'
    path = r"C:\Users\joshu\Documents\ArtificialIntelligence\MetaSHAP\models\model94.pth"
    ...
    train_idx1 = torch.tensor(train_dataset.targets) == 4
    train_idx2 = torch.tensor(train_dataset.targets) == 9
    ...
    test_idx1 = torch.tensor(test_dataset.targets) == 4
    test_idx2 = torch.tensor(test_dataset.targets) == 9
    ...
    background = images[:1000].to(device)    
